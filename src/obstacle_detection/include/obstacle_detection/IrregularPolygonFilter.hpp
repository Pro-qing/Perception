...
public:
     ...
     *
     * 启用后，在applyFilter/applyFilterNegative中，多边形外的点
     * 如果强度在指定范围内也会被保留（排除强度值为9.0的特殊点）
     */
     ...
     *
     * 如果启用了强度滤波，多边形外但强度在范围内的点也会被保留
     * 注意：强度值恰好为9.0的点会被排除（fabs(intensity - 9.00) > 0.01）
     */
