...
DownsampleProcessor::DownsampleProcessor(ros::NodeHandle& nh, ros::NodeHandle& pnh)
    : nh_(nh), pnh_(pnh)
{
...
bool DownsampleProcessor::processDownsample(pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud) {
    if (cloud->empty()) {
        return false;
    }
...
// ============== 加载车身多边形顶点 ==============
void DownsampleProcessor::loadBodyPolygon() {
    XmlRpc::XmlRpcValue polygon_list;
    if (!pnh_.getParam("body_filter/polygon", polygon_list)) {
        ROS_WARN("[Downsample] No body_filter/polygon found. Using empty polygon.");
        return;
    }

    if (polygon_list.getType() != XmlRpc::XmlRpcValue::TypeArray) {
        ROS_WARN("[Downsample] body_filter/polygon is not an array.");
        return;
    }
...
bool DownsampleProcessor::isPointInsidePolygon(double px, double py) const {
...
// ============== 发布车身 MarkerArray ==============
void DownsampleProcessor::publishBodyMarker() {
    visualization_msgs::MarkerArray marker_array;

    // ---- Marker 1: 线框 (LINE_LIST) ----
    ...
    ROS_INFO("[Downsample] Body MarkerArray published on %s (%d markers)",
